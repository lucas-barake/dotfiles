...
- inspect installed package metadata and official library source and tests in `~/src/oss/` when the plan depends on third party library or framework testing patterns. Use metadata fields such as `repository.url`, `repository.directory`, exports, and version to find the matching official repository and package directory. If the repo is missing, shallow clone it there first. Use those tests to validate planned harnesses, composition, setup, and assertions. This applies to any library, including Effect and Effect ecosystem packages
...
7. Check that every planned third party library or framework test follows the matching official repository and package directory test patterns from `~/src/oss/`, not memory or generic examples
