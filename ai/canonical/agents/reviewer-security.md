...
5. For each candidate bug, use the TDD fix workflow. Before writing a regression/security test that depends on third party library or framework behavior, inspect installed package metadata for the official repository URL, package directory, exports, and version. Then inspect version matched official source and tests under `~/src/oss/`, reusing an existing shared version checkout if the main checkout is on a different version, and follow its test harness, composition, setup, and assertion patterns. This applies to any library. Write the smallest regression/security test that should fail because of the suspected bug. Prefer existing nearby test files and conventions. Use a concrete malicious input and assert the secure behavior.
