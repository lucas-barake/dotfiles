...
2. Inspect installed package metadata and lockfiles for each relevant library or framework. Use `repository.url`, `repository.directory`, exports, and installed version to find the official repository and package directory under `~/src/oss/`, then inspect the matching upstream tag, release branch, or commit
3. If the shared checkout is on a different version, first look for an existing reusable version checkout under `~/src/oss/.versions/<repo>/<version>/` or another clearly named shared version directory. Create one shared isolated git worktree or clone only if that exact version is missing. If no matching upstream ref exists, use the installed package source as the version source of truth and record the mismatch
