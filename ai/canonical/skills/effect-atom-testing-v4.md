...
- `vitest.useFakeTimers()` / `vitest.useRealTimers()`: **NOT TestClock**.
  Atom internals use real `setTimeout`/`setInterval`, so use vitest's
  fake timers
...
For atoms backed by Effects (runtime atoms, `Atom.fn`, streams), **mount**
the atom:
...
Atoms must NEVER directly consume services with complex inferred types
(e.g., RPC clients). RPC clients have auto-generated types that are
impossible to mock with `Layer.mock`.
...
- `scheduleTask={(f) => f()}`: makes atom updates synchronous in tests
