...
description: "Effect v4 Config and ConfigProvider patterns for declarative, schema-driven configuration loading from env vars, JSON objects, .env files, and directory trees. Use when working with Config, ConfigProvider, Config.schema, Config.all, Config.withDefault, Config.nested, ConfigProvider.fromEnv, ConfigProvider.fromUnknown, ConfigProvider.fromDotEnv, ConfigProvider.constantCase, or any config-related code in Effect v4. Triggers on Config.string, Config.number, Config.boolean, Config.int, Config.port, Config.schema, Config.all, Config.withDefault, Config.option, Config.nested, Config.redacted, Config.url, Config.date, Config.duration, Config.logLevel, Config.map, Config.mapOrFail, Config.orElse, Config.unwrap, Config.succeed, Config.fail, Config.make, ConfigProvider, ConfigProvider.fromEnv, ConfigProvider.fromUnknown, ConfigProvider.fromDotEnv, ConfigProvider.fromDotEnvContents, ConfigProvider.fromDir, ConfigProvider.constantCase, ConfigProvider.nested, ConfigProvider.mapInput, ConfigProvider.orElse, ConfigProvider.layer, ConfigProvider.layerAdd, ConfigProvider.make, ConfigError."
---
...
All convenience constructors delegate to `Config.schema`. It takes a
...
Signature: `schema<T, E>(codec: Schema.Codec<T, E>, path?: string | Path): Config<T>`
...
Every constructor takes an optional `name` parameter that sets the root path
...
Provide a lazy fallback value on MISSING DATA ONLY. Validation errors
...
```ts
provider.pipe(ConfigProvider.mapInput((path) =>
...
- Composite/AnyOf where ALL sub-issues are missing data
...
```ts
const config = Config.schema(Schema.Struct({
  host: Schema.String,
  port: Schema.Int
}), "db")
...
| `Config.withDefault(value)` | `Config.withDefault(() => value)` (lazy) |
