...
class HttpClient extends ServiceMap.Service<HttpClient>()("HttpClient", {
  make: (baseUrl: string) => Effect.gen(function*() {
...
```ts
const verbose = Layer.succeed(LogLevel, "debug")
```
...
service implementation. Automatically strips `Scope` from requirements (no
separate `Layer.scoped` needed):

```ts
const layer = Layer.effect(Database, Effect.gen(function*() {
...
### Memoization is Per-Build

Each `Layer.build`, `Effect.provide(layer)`, or `it.layer()` call creates a
fresh MemoMap. Different builds never share memoized results.
...
Each `it.layer()` block creates a new MemoMap. All tests within the block
share the same layer instance:
