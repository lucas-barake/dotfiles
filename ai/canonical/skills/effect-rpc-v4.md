...
```ts
import { Rpc, RpcGroup, RpcClient, RpcServer, RpcMiddleware, RpcSerialization, RpcSchema, RpcTest, RpcClientError } from "effect/unstable/rpc"
import { Effect, Layer, Schema, ServiceMap, Stream, Queue, Option, Headers } from "effect"
```
...
client.StreamUsers({ id: "1" }, { asQueue: true }) => Effect<Queue.Dequeue<User, Never>, never, Scope>
```
...
| `RpcServer.layerProtocolStdio` | Stdin/stdout | `RpcSerialization` |
...
In v4, `Layer.succeed`, `Layer.effect`, and `Layer.scoped` use curried form:
